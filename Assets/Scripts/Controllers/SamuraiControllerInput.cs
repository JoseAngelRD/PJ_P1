...
public class SamuraiControllerInput : CharacterController
{        
    // Update is called once per frame
    void Update()
    {
        // Distancias
        Debug.DrawRay(transform.position, Vector3.right * transform.localScale.x * dashForce * dashDuration, Color.red);
        if (FindObjectOfType<PlayerController>().transform.position.x - Mathf.Abs((Vector3.right * transform.localScale.x * dashForce * dashDuration).x) <= 0)
        {
            Debug.Log("EN RANGO DASH");
        }
        // Estado del player
